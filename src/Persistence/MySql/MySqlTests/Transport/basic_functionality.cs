...
public class basic_functionality : IAsyncLifetime
{
    ...
    private MySqlTransport theTransport = null!;
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        try
        {
            ...
            dropCmd.CommandText = "DROP DATABASE IF EXISTS wolverine_transports; CREATE DATABASE wolverine_transports;";
        ...
        theHost = await Host.CreateDefaultBuilder()
            .UseWolverine(opts =>
            {
                opts.UseMySqlPersistenceAndTransport(Servers.MySqlConnectionString, schema: "wolverine_transports",
                    transportSchema: "wolverine_transports");
                opts.ListenToMySqlQueue("one");
            }).StartAsync();

        theTransport = theHost.GetRuntime().Options.Transports.GetOrCreate<MySqlTransport>();
...
AND table_name LIKE 'wolverine_queue_%'";
        ...
        tables.Any(x => x == "wolverine_queue_one").ShouldBeTrue();
        tables.Any(x => x == "wolverine_queue_one_scheduled").ShouldBeTrue();
    }
    ...
    public async Task delete_expired_smoke_test()
    {
        ...
        await using var theListener = new MySqlQueueListener(theQueue, theRuntime, durableReceiver, theQueue.DataSource, null);
    ...
    public async Task move_from_outgoing_to_queue_async()
    {
        ...
        await new MySqlQueueSender(theQueue).MoveFromOutgoingToQueueAsync(envelope, CancellationToken.None);
    ...
    public async Task move_from_outgoing_to_scheduled_async()
    {
        ...
        await new MySqlQueueSender(theQueue).MoveFromOutgoingToScheduledAsync(envelope, CancellationToken.None);
    ...
    public async Task move_from_scheduled_to_queue()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            await new MySqlQueueSender(theQueue).ScheduleMessageAsync(envelope, CancellationToken.None);
        }
